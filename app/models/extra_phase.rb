class ExtraPhase < ActiveRecord::Base
  ...
  def set_points_for_score_boards
    if self.score_board.id == ScoreBoard.main_score_board.id
      ...
      @score_boards.each do |score_board|
        ...
        next if extra.nil?
        if extra.best_player_id == main_extra.best_player_id
          score_board.points += 7
        end
        if extra.red_card_team_id == main_extra.red_card_team_id
          score_board.points += 5
        end
        if extra.penal_team_id == main_extra.penal_team_id
          score_boards.points += 3
        end
        score_board.save
      end
    end
  ...
  def erase_points_for_score_boards
    if self.score_board.id == ScoreBoard.main_score_board.id
      @score_boards =  ScoreBoard.not_main_board.active
      main_extra = self
      old_main_extra = ExtraPhase.find(self.extra_phase.id)
      @score_boards.each do |score_board|
        extra = score_board.extra_phase
        next if extra.nil?
        if extra.best_player_id == main_extra.best_player_id
